 ...
 *      using a depth first search.     */
{
    ...
    while (!done && !(err = scan(pstate->IP, &Icode))) {
        ...
        if (pProc->Icode.labelSrch(Icode.ic.ll.label,
                                   &lab)) { /* Synthetic jump */
        ...
        case iJCXZ: {
            ...
            if (ip > 0 && prev->ic.ll.opcode == iCMP && (prev->ic.ll.flg & I)) {
                ...
                fBranch = (boolT)(Icode.ic.ll.opcode == iJB ||
                                  Icode.ic.ll.opcode == iJBE);
            }
            ...
            /* Next icode. Note: not the same as GetLastIcode() because of the
               call to FollowCtrl() */
...
/* process_JMP - Handles JMPs, returns TRUE if we should end recursion  */
{
    ...
    if (pIcode->ic.ll.opcode == iJMP && (pIcode->ic.ll.flg & WORD_OFF) &&
        ...
        if (offTable < endTable) {
            ...
            psw = (dword *)allocMem(pIcode->ic.ll.caseTbl.numEntries *
                                    sizeof(dword));
    ...
    if (!(pIcode->ic.ll.flg & I)) {
        ...
        if (pIcode->ic.ll.opcode == iCALLF)
            pIcode->ic.ll.immed.op =
                LH(&prog.Image[off]) + ((dword)(LH(&prog.Image[off + 2])) << 4);
        else
            pIcode->ic.ll.immed.op =
                LH(&prog.Image[off]) + ((dword)(word)pProc->state.r[rCS] << 4);
        pIcode->ic.ll.flg |= I;
        indirect = TRUE;
    }
