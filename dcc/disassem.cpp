...
static char *szOps[] = {
    ...
    "REPE CMPS", "DAA",      "DAS",        "DEC",       "DIV",  "ENTER",
    "ESC",       "HLT",      "IDIV",       "IMUL",      "IN",   "INC",
    "INS",       "REP INS",  "INT",        "IRET",      "JB",   "JBE",
    "JAE",       "JA",       "JE",         "JNE",       "JL",   "JGE",
    "JLE",       "JG",       "JS",         "JNS",       "JO",   "JNO",
    "JP",        "JNP",      "JCXZ",       "JMP",       "JMP",  "LAHF",
    "LDS",       "LEA",      "LEAVE",      "LES",       "LOCK", "LODS",
    "REP LODS",  "LOOP",     "LOOPE",      "LOOPNE",    "MOV",  "MOVS",
    "REP MOVS",  "MUL",      "NEG",        "NOT",       "OR",   "OUT",
    "OUTS",      "REP OUTS", "POP",        "POPA",      "POPF", "PUSH",
    "PUSHA",     "PUSHF",    "RCL",        "RCR",       "ROL",  "ROR",
    "RET",       "RETF",     "SAHF",       "SAR",       "SHL",  "SHR",
    "SBB",       "SCAS",     "REPNE SCAS", "REPE SCAS", "CWD",  "STC",
    "STD",       "STI",      "STOS",       "REP STOS",  "SUB",  "TEST",
    "WAIT",      "XCHG",     "XLAT",       "XOR",       "INTO", "NOP",
    "REPNE",     "REPE",     "MOD"};
...
static char *szFlops1[] = {
    ...
    "FLD",     "???",    "FST",    "???",
    "FLDENV",  "FLDCW",  "FSTENV", "FSTSW", /* 08 */
    "FIADD",   "FIMUL",  "FICOM",  "FICOMP",
    "FISUB",   "FISUBR", "FIDIV",  "FIDIVR", /* 10 */
    "FILD",    "???",    "FIST",   "FISTP",
    "???",     "???",    "???",    "FSTP", /* 18 */
    "FADD",    "FMUL",   "FCOM",   "FCOMP",
    "FSUB",    "FSUBR",  "FDIV",   "FDIVR", /* 20 */
    "FLD",     "FLD",    "FST",    "FSTP",
    "FRESTOR", "???",    "FSAVE",  "FSTSW", /* 28 */
    "FIADD",   "FIMUL",  "FICOM",  "FICOMP",
    "FISUB",   "FISUBR", "FIDIV",  "FIDIVR", /* 30 */
    "FILD",    "???",    "FIST",   "FISTP",
    "FBLD",    "???",    "FBSTP",  "FISTP" /* 38 */
};
...
static void dis1Line(Int i, boolT fWindow, char attr, Int pass) {
    ...
    case iREP_OUTS:
        if (pIcode->ic.ll.src.segOver) {
            (pIcode->ic.ll.opcode == iOUTS || pIcode->ic.ll.opcode == iREP_OUTS)
                ? strcat(strcpy(p + WID_PTR, "dx, "),
                         szPtr[pIcode->ic.ll.flg & B])
    ...
    case iIN:
        ...
        strcat(p + WID_PTR,
               (pIcode->ic.ll.flg & I) ? strHex(pIcode->ic.ll.immed.op) : "dx");
        break;
    ...
    if (readVal(cbuf, pIcode->ic.ll.label, 0)) {
        ...
        strcpy(buf + POS_CMT + 1, cbuf);
             ...
             (pIcode->ic.ll.flg &
              (SWITCH | CASE | SEG_IMMED | IMPURE | SYNTHETIC | TERMINATES))) {
        buf[strlen(buf)] = ' ';
        buf[POS_CMT] = '\0';
    ...
    if (!(pIcode->ic.ll.flg & SYNTHETIC)) {
        if (fWindow) {
            ...
            off =
                (word)(pIcode->ic.ll.label - ((dword)pProc->state.r[rCS] << 4));
            attrSet(attr);
...
void updateScr(boolT bNew) {
    ...
    if (bNew) {
        ...
        for (x = 0; x < NSCROLL; x++) {
            if (icTop && pc[icTop - 1].ic.ll.label +
                                 (dword)pc[icTop - 1].ic.ll.numBytes ==
    ...
    for (y = 1, ic = icTop; y < LINES - 1; ic++, y++) {
        ...
        // Only have to repaint if screen is new, or repainting formerly
        // highlighted line, or newly highlighted line
        if (bNew || (pcCur == nextInst) || (oldPcCur == nextInst))
            dis1Line(ic, TRUE,
                     (char)((pcCur == nextInst) ? A_REVERSE : A_NORMAL), 0);
...
static Int checkScanned(dword pcCur) {
    ...
    if (!labelSrch(pc, numIcode, pcCur, &i)) {
        ...
        if (numIcode >= allocIcode) {
            allocIcode =
                numIcode +
                DELTA_ICODE; /* Make space for this one, and a few more */
            ...
            memset(&pc[numIcode], 0,
                   (size_t)(allocIcode - numIcode) * sizeof(ICODE));
            pl = (Int *)reallocVar(pl, allocIcode * sizeof(Int));
            memset(&pl[numIcode], 0,
                   (size_t)(allocIcode - numIcode) * sizeof(Int));
        }
        i = numIcode++;
    }
...
void interactDis(PPROC initProc, Int initIC) {
    ...
    while (fInteract) {
        ...
        // numeric keypad keys work as well (regardless of numlock state).
        oldPcCur = pcCur;
        switch (ch) {
        case KEY_DOWN:
        ...
        case KEY_UP:
            ...
            if ((icCur == 0) ||
                pc[--icCur].ic.ll.label + (dword)pc[icCur].ic.ll.numBytes !=
                    pcCur) {
                for (i = 0; i < numIcode; i++) {
                    if (pc[i].ic.ll.label + (dword)pc[i].ic.ll.numBytes ==
                        pcCur) {
        ...
        case KEY_PPAGE:
            ...
            for (i = 0; i < numIcode; i++) {
                if ((pc[i].ic.ll.label <= pcTop) &&
                    (pc[i].ic.ll.label + (dword)pc[i].ic.ll.numBytes >=
                     pcTop)) {
...
static void flops(PICODE pIcode) {
    ...
    if ((pIcode->ic.ll.dst.regi == 0) ||
        (pIcode->ic.ll.dst.regi >= INDEXBASE)) {
        ...
        /* The mod/rm mod bits are set to 11 (i.e. register).
           ...
           for this case (e.g. op=04 means FSUB if reg != 3, but FSUBR for reg
           == 3), a separate table is used (szFlops2). */
        switch (op) {
        case 0x0C:
