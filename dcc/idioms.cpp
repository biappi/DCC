...
static Int idiom3(PICODE pIcode, PICODE pEnd)
{
    ...
    else if ((pIcode->ic.ll.opcode == iMOV) &&
             (pIcode->ic.ll.dst.regi == rSP) && (pIcode->ic.ll.src.regi == rBP))
    ...
    if ((pIcode + 2) < pEnd) {
        ...
        if ((type != CONSTANT) && (type != OTHER)) {
            ...
            if (((pIcode + 1)->ic.ll.opcode == iNEG) &&
                ...
                switch (type) {
                case GLOB_VAR:
                    if (((pIcode + 2)->ic.ll.dst.segValue ==
 ...
 *			against a register, variable or constant different than
 *0). INC var			or DEC var (including register vars) MOV reg,
 *var	   MOV reg, var CMP reg, Y		   CMP reg, Y
 *		JX  lab			   JX  lab
    ...
    while (pIcode < pEnd) {
        ...
        case iINC:
            if (idiom18(pIcode, pEnd, pProc)) {
                ...
                exp = boolCondExp(
                    lhs, rhs, condOpJCond[(pIcode + 1)->ic.ll.opcode - iJB]);
        ...
        case iCALLF:
            ...
            if (pIcode->ic.ll.immed.proc.proc != 0)
                if ((pIcode->ic.ll.immed.proc.proc->flg & PROC_ISLIB) &&
                    ...
                    if ((pIcode->ic.ll.immed.proc.proc->retVal.type ==
                         TYPE_LONG_SIGN) ||
