...
/* Indentation buffer */
#define indSize                                                                \
    81 /* size of the indentation buffer.  Each indentation                    \
...
/* displays statistics on the subroutine */
{
    ...
    if (!(pProc->flg & PROC_ASM)) {
        ...
        printf("  Percentage reduction: %2.2f%%\n",
               100.0 - (stats.numHLIcode * 100.0) / stats.numLLIcode);
    }
}
 ...
 *		exe file: prog.Image[operand+0x100] 	*/
{
    ...
    switch (psym->size) {
    case 1:
        appendStrTab(&cCode.decl, "byte\t%s = %ld;\n", psym->name,
                     prog.Image[relocOp]);
        break;
    case 2:
        appendStrTab(&cCode.decl, "word\t%s = %ld;\n", psym->name,
                     LH(prog.Image + relocOp));
        break;
    case 4:
        if (psym->type == TYPE_PTR) /* pointer */
            appendStrTab(&cCode.decl, "word\t*%s = %ld;\n", psym->name,
                         LH(prog.Image + relocOp));
        else /* char */
            appendStrTab(&cCode.decl, "char\t%s[4] = \"%c%c%c%c\";\n",
                         psym->name, prog.Image[relocOp],
                         prog.Image[relocOp + 1], prog.Image[relocOp + 2],
                         prog.Image[relocOp + 3]);
        break;
    default:
        strContents = (char *)allocMem((psym->size * 2 + 1) * sizeof(char));
        strContents[0] = '\0';
 ...
 */
{
    ...
    if (loopType) {
        ...
        case WHILE_TYPE:
            ...
            if (pBB->numHlIcodes > 1) {
                ...
                writeBB(pBB, pProc->Icode.GetFirstIcode(), indLevel, pProc,
                        numLoc);
                repCond = TRUE;
            }
    ...
    if (loopType) /* there is a loop */
    {
        ...
        if (loopType == WHILE_TYPE) {
            ...
            if (repCond)
                writeBB(pBB, pProc->Icode.GetFirstIcode(), indLevel + 1, pProc,
                        numLoc);
            ...
            if (pBB->ifFollow != MAX) /* there is a follow */
            {
                ...
                if (succ->traversed != DFS_ALPHA) /* not visited */
                {
                    ...
                    } else /* empty THEN part => negate ELSE part */
                    {
                        ...
                        appendStrTab(&cCode.code, "\n%s%s",
                                     indent(indLevel - 1), l);
                        writeCode(pBB->edges[ELSE].BBptr, indLevel, pProc,
