...
dword maskDuReg[] = {
    0x00,     0xFEEFFE, 0xFDDFFD, 0xFBB00B, 0xF77007, /* word regs */
...
/* Places the new HLI_POP/HLI_PUSH/HLI_RET high-level operand in the high-level
 * icode array */
{
    pIcode->type = HIGH_LEVEL;
 ...
 * removed) */
{
    ...
    if (numDefs == thisDefIdx)
        for (; numDefs > 0; numDefs--) {
            if ((picode->du1.idx[numDefs - 1][0] != 0) ||
                (picode->du.lastDefRegi))
                break;
        }
 ...
 *       refines the HIGH_LEVEL icodes. */
{
    ...
    for (i = 0; i < numIcode; i++) {
        ...
        if ((pIcode->type == LOW_LEVEL) && (pIcode->invalid == FALSE)) {
            ...
            case iRET:
            case iRETF:
                newUnaryHlIcode(pIcode, HLI_RET, NULL);
                break;
 ...
 * then part. 	*/
{
    static condOp invCondOp[] = {
        GREATER, GREATER_EQUAL, NOT_EQUAL, EQUAL,  LESS_EQUAL, LESS,  DUMMY,
        DUMMY,   DUMMY,         DUMMY,     DUMMY,  DUMMY,      DUMMY, DUMMY,
        DUMMY,   DUMMY,         DUMMY,     DBL_OR, DBL_AND};
    if (*exp == NULL)
        return;
