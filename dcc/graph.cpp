...
PBB createCFG(PPROC pProc)
{
    ...
    for (ip = start = 0; pProc->Icode.IsValid(pIcode); ip++, pIcode++) {
        ...
        if (ip + 1 == pProc->Icode.GetNumIcodes() &&
            !(pIcode->ic.ll.flg & TERMINATES) && pIcode->ic.ll.opcode != iJMP &&
            pIcode->ic.ll.opcode != iJMPF && pIcode->ic.ll.opcode != iRET &&
            pIcode->ic.ll.opcode != iRETF)
        ...
        /* Only process icodes that have valid instructions */
            ...
            default:
                ...
                else if (pProc->Icode.GetFirstIcode()[ip + 1].ic.ll.flg &
                         (TARGET | CASE)) {
...
void compressCFG(PPROC pProc)
{
    ...
    for (pBB = pProc->cfg; pBB; pBB = pBB->next)
        if (pBB->numInEdges != 0 &&
            (pBB->nodeType == ONE_BRANCH || pBB->nodeType == TWO_BRANCH))
            for (i = 0; i < pBB->numOutEdges; i++) {
    ...
    if (pBB) {
        while (pBB->nodeType == FALL_NODE || pBB->nodeType == ONE_BRANCH) {
            ...
            if (pBB->nodeType == ONE_BRANCH) {
                ...
                for (i = ip;
                     i < pChild->start && (pProc->Icode.GetLlFlag(i) & NO_CODE);
                     i++)
                    ;
                if (i != pChild->start)
                    break;
