...
COND_EXPR *idCondExpLong(LOCAL_ID *localId, opLoc sd, PICODE pIcode, hlFirst f,
                         Int ix, operDu du, Int off)
/* Returns an identifier conditional expression node of type LONG_VAR */
{
    COND_EXPR *newExp;
    Int idx;
    ...
    if ((sd == SRC) && ((pIcode->ic.ll.flg & I) == I)) /* constant */
    {
        ...
        if (f == HIGH_FIRST)
            newExp->expr.ident.idNode.kte.kte =
                (pIcode->ic.ll.immed.op << 16) + (pIcode + off)->ic.ll.immed.op;
        else /* LOW_FIRST */
            newExp->expr.ident.idNode.kte.kte =
 ...
 *            du: operand is defined or used in current instruction.    */
{
    ...
    else if ((sd == DST) && (pIcode->ic.ll.flg & IM_TMP_DST) ==
                                IM_TMP_DST) { /* implicit tmp */
    ...
    {
        newExp = idCondExpReg(pm->regi,
                              (sd == SRC) ? pIcode->ic.ll.flg
    ...
    else /* (pm->regi >= INDEXBASE && pm->off = 0) => indexed && no off */
    {
        if ((pm->seg == rDS) && (pm->regi > INDEXBASE + 3)) /* dereference */
        {
            switch (pm->regi) {
            case INDEXBASE + 4:
                newExp = idCondExpReg(rSI, 0, &pProc->localId);
                setRegDU(duIcode, rSI, du);
                break;
            case INDEXBASE + 5:
                newExp = idCondExpReg(rDI, 0, &pProc->localId);
                setRegDU(duIcode, rDI, du);
                break;
            case INDEXBASE + 6:
                newExp = idCondExpReg(rBP, 0, &pProc->localId);
                break;
            case INDEXBASE + 7:
                newExp = idCondExpReg(rBX, 0, &pProc->localId);
    ...
    case IDENTIFIER:
        ...
        case FUNCTION:
            o = writeCall(exp->expr.ident.idNode.call.proc,
                          exp->expr.ident.idNode.call.args, pProc, numLoc);
            break;
