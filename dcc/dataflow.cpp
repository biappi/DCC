...
{
    Int i, useAt,   /* Index to instruction that used flag    */
                    ...
                    if ((use & def) == use) {
                        ...
                        if ((pProc->Icode.GetLlOpcode(useAt - 1) >= iJB) &&
                            ...
                            if (!notSup) {
                                exp = boolCondExp(
                                    lhs, rhs,
                                    condOpJCond[pProc->Icode.GetLlOpcode(useAt -
                                                                         1) -
                                                iJB]);
                                newJCondHlIcode(
                                    pProc->Icode.GetIcode(useAt - 1), exp);
                            }
                        }
            ...
            if (pbb->numOutEdges == 0) /* HLI_RET node         */
            {
                ...
                if (pproc->flg & PROC_IS_FUNC) {
                    ...
                    if (picode->ic.hl.opcode == HLI_RET) {
                        picode->ic.hl.oper.exp =
                            idCondExpID(&pproc->retVal, &pproc->localId,
                ...
                for (k = 0; k < INDEXBASE; k++) {
                    if ((picode->du.def & power2(k)) != 0) {
                        ...
                        if ((picode->ic.hl.opcode == HLI_CALL) &&
                            ...
                            useIdx = 0;
                            for (n = tbb->start; n < tbb->start + tbb->length;
                                 n++) {
                                ticode = pProc->Icode.GetIcode(n);
                                if (ticode->type == HIGH_LEVEL) {
                                    /* if used, get icode index */
                        ...
                        if ((picode->invalid == FALSE) &&
                            ...
                            if (!(pbb->liveOut & duReg[regi])) /* not liveOut */
                            {
                                ...
                                /* Backpatch any uses of this instruction,
                                 * within the same BB, if the instruction was
                                 * invalidated */
                                if (res == TRUE)
                ...
                if (picode->du1.numRegsDef == 1) /* byte/word regs */
                {
                    ...
                    if ((picode->du1.idx[0][0] != 0) &&
                        ...
                        case HLI_ASSIGN:
                             ...
                             * icode expression */
                            ticode =
                                pProc->Icode.GetIcode(picode->du1.idx[0][0]);
                        ...
                        case HLI_POP:
                            ...
                            switch (ticode->ic.hl.opcode) {
                            case HLI_ASSIGN:
                                forwardSubs(picode->ic.hl.oper.exp, exp, picode,
                                            ticode, &pProc->localId,
                                            &numHlIcodes);
                                break;
                        ...
                        case HLI_CALL:
                            ...
                            case HLI_ASSIGN:
                                exp =
                                    idCondExpFunc(picode->ic.hl.oper.call.proc,
                                                  picode->ic.hl.oper.call.args);
                                res = insertSubTreeReg(
                                    exp, &ticode->ic.hl.oper.asgn.rhs,
                                    picode->ic.hl.oper.call.proc->retVal.id
                                        .regi,
                                    &pProc->localId);
                                if (!res)
                                    insertSubTreeReg(
                                        exp, &ticode->ic.hl.oper.asgn.lhs,
                                        picode->ic.hl.oper.call.proc->retVal.id
                                            .regi,
                                        &pProc->localId);
                                /***  HERE missing: 2 regs ****/
                ...
                else if (picode->du1.numRegsDef == 2) /* long regs */
                {
                    ...
                    if ((picode->du1.idx[0][0] != 0) &&
                        ...
                        case HLI_ASSIGN:
                            /* Replace rhs of current icode into target
                             * icode expression */
                            if (picode->du1.idx[0][0] ==
                                picode->du1.idx[1][0]) {
                                ticode = pProc->Icode.GetIcode(
                                    picode->du1.idx[0][0]);
                                if ((picode->du.lastDefRegi & duReg[regi]) &&
                                    ((ticode->ic.hl.opcode != HLI_CALL) &&
                                     (ticode->ic.hl.opcode != HLI_RET)))
                                ...
                                case HLI_ASSIGN:
                                    forwardSubsLong(
                                        picode->ic.hl.oper.asgn.lhs->expr.ident
                                            .idNode.longIdx,
                        ...
                        case HLI_POP:
                            if (picode->du1.idx[0][0] ==
                                ...
                                switch (ticode->ic.hl.opcode) {
                                case HLI_ASSIGN:
                                    forwardSubsLong(picode->ic.hl.oper.exp->expr
                                                        .ident.idNode.longIdx,
                                                    exp, picode, ticode,
                                                    &numHlIcodes);
                                    break;
                                case HLI_JCOND:
