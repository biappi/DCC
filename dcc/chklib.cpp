...
boolT LibCheck(PPROC pProc)
{
    ...
    if (locatePattern(prog.Image, pProc->procEntry,
                      pProc->procEntry + sizeof(pattMsChkstk), pattMsChkstk,
...
void checkStartup(STATE *pState)
{
    ...
    /* Check the Turbo Pascal signatures first, since they involve only the
            first 3 bytes, and false positives may be founf with the others
       later */
    ...
    if (locatePattern(prog.Image, startOff, startOff + 0x180, pattMainLarge,
        ...
        para =
            LH(&prog.Image[i + OFFMAINLARGE + 2]); /* This is abs seg of main */
    ...
    if (memcmp(&prog.Image[startOff], pattMsC5Start, sizeof(pattMsC5Start)) ==
        0) {
        /* Yes, this is Microsoft startup code. The DS is sitting right here
            in the next 2 bytes */
        setState(pState, rDS,
                 LH(&prog.Image[startOff + sizeof(pattMsC5Start)]));
    ...
    else if (memcmp(&prog.Image[startOff], pattMsC8Start,
                    sizeof(pattMsC8Start)) == 0) {
        setState(pState, rDS,
                 LH(&prog.Image[startOff + sizeof(pattMsC8Start)]));
    ...
    /* The C8 .com startup pattern is different again! */
    ...
    else if (locatePattern(prog.Image, startOff, startOff + 0x30,
                           pattBorl2Start, sizeof(pattBorl2Start), &i)) {
        ...
        chVersion = '2'; /* Version 2 */
                           ...
                           pattBorl3Start, sizeof(pattBorl3Start), &i)) {
        /* Borland startup. DS is at the second byte (offset 1) */
        setState(pState, rDS, LH(&prog.Image[i + 1]));
...
void readProtoFile(void)
{
    ...
    for (i = 0; i < numArg; i++) {
        //      fread(&pArg[i], 1, SYMLEN, fProto);     /* No names to read as
        //      yet */
