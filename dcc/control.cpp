...
#define ancestor(a, b)                                                         \
    ((a->dfsLastNum < b->dfsLastNum) && (a->dfsFirstNum < b->dfsFirstNum))
 ...
 * Note: graph should be reducible */
{
    ...
    for (currIdx = 0; currIdx < pProc->numBBs; currIdx++) {
        ...
        for (j = 0; j < currNode->numInEdges; j++) {
            ...
            if (predIdx < currIdx)
                currNode->immedDom =
                    commonDom(currNode->immedDom, predIdx, pProc);
        }
    }
}
...
static void findNodesInLoop(PBB latchNode, PBB head, PPROC pProc,
                            queue *intNodes)
        ...
        if ((currNode->nodeType == TWO_BRANCH) &&
            ...
            for (desc = curr + 1; desc < pProc->numBBs; desc++) {
                if (pProc->dfsLast[desc]->immedDom == curr) {
                    ...
                    if ((pbb->numInEdges - pbb->numBackEdges) >=
                        followInEdges) {
                        follow = desc;
