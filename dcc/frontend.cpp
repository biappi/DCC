    ...
    for (pProc = pProcList; pProc; pProc = pProc->next) {
        for (i = 0; i < pProc->Icode.GetNumIcodes(); i++) {
            if (pProc->Icode.GetLlFlag(i) & (SYM_USE | SYM_DEF)) {
                psym = &symtab.sym[pProc->Icode.GetIcode(i)
                                       ->ic.ll.caseTbl.numEntries];
                for (c = (Int)psym->label; c < (Int)psym->label + psym->size;
                     c++) {
                    if (BITMAP(c, BM_CODE)) {
...
static void LoadImage(char *filename) {
    ...
    if (!(prog.fCOM = (boolT)(header.sigLo != 0x4D || header.sigHi != 0x5A))) {
         ...
         */
        cb = (dword)LH(&header.numPages) * 512 -
             (dword)LH(&header.numParaHeader) * 16;
        ...
        if (prog.cReloc) {
            ...
            for (i = 0; i < prog.cReloc; i++) {
                fread(buf, 1, 4, fp);
                prog.relocTable[i] =
                    LH(buf) + (((Int)LH(buf + 2) + EXE_RELOCATION) << 4);
            }
        }
